...
IActiveConditionProcessor::~IActiveConditionProcessor()
{
}
...
{
    // Expect all signals to be small (below or equal MAX_SIGNAL_ID) otherwise this will fail because not enough
    // memory being available
    if ( mSignalBuffers.size() <= signal.signalID )
    {
        // to avoid too many calls to resize every time increase is needed increase at least
        // SIGNAL_BUFFER_INCREASE_STEPS All the elements will be default initialized
        mSignalBuffers.resize( std::max<uint32_t>(
            signal.signalID + 1, SIGNAL_BUFFER_INCREASE_STEPS + static_cast<uint32_t>( mSignalBuffers.size() ) ) );
    }
    ...
    const std::shared_ptr<const InspectionMatrix> &activeInspectionMatrix )
{
    ...
    mSignalBuffers.resize( SIGNAL_BUFFER_INCREASE_STEPS ); // signal IDs must be continuous max 50.000
    ...
    for ( auto &p : mActiveInspectionMatrix->conditions )
    {
        ...
        ActiveCondition &currentCondition = mConditions.back();
        size_t conditionIndex = mConditions.size() - 1;
        for ( auto &s : p.signals )
        {
            if ( s.signalID > MAX_SIGNAL_ID )
            {
                ...
                mLogger.error( "CollectionInspectionEngine::onChangeInspectionMatrix",
                               "The signalID " + std::to_string( s.signalID ) +
                                   " is out of range. Only signal IDs less or equal to" +
                                   std::to_string( MAX_SIGNAL_ID ) + " are accepted. Skipping" );
                return;
            }
            ...
            FixedTimeWindowFunctionData *window = buf.addFixedWindow( s.fixedWindowPeriod );
            if ( isSignalPartOfEval( p.condition, s.signalID, MAX_EQUATION_DEPTH ) )
            {
                buf.mConditionsThatEvaluateOnThisSignal.set( conditionIndex );
                currentCondition.mEvaluationSignals[s.signalID] = &buf;
                currentCondition.mEvaluationFunctions[s.signalID] = window;
            }
        }
...
{
    if ( id >= mSignalBuffers.size() || mSignalBuffers[id].size() == 0 )
    {
        // Signal not collected by any active condition
        return;
    }
