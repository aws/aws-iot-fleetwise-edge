...
OBDOverCANSessionManager::OBDOverCANSessionManager()
{
}
...
OBDOverCANSessionManager::init( const std::string &gatewayCanInterfaceName )
{
    ...
    ISOTPOverCANSenderReceiverOptions options( gatewayCanInterfaceName, BROADCAST_ID, ENGINE_ECU_RX );
    // Initialize the request
    OBDRequest KeepAliveOBDRequest = { static_cast<SID>( 0x01 ), static_cast<PID>( 0x00 ) };
    mTxPDU.clear();
    mTxPDU.emplace_back( KeepAliveOBDRequest.mSID );
