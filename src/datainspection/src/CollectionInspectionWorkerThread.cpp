...
CollectionInspectionWorkerThread::CollectionInspectionWorkerThread()
{
    ...
    // During init, there is no matrix available
    // This will allow the thread to start safely
    fUpdatedInspectionMatrixAvailable = false;
    fShouldStop = false;
}
...
CollectionInspectionWorkerThread::doWork( void *data )
{
    ...
    timestampT lastInputTimeEvaluated = 0;
    timestampT lastTimeEvaluated = 0;
    timestampT lastTraceOutput = 0;
    ...
    do
    {
        ...
        else
        {
            ...
            consumer->fWait.wait( Platform::Signal::WaitWithPredicate );
        }
