...
private:
    ...
    static const uint32_t SIGNAL_BUFFER_INCREASE_STEPS =
        1000; // Only necessary until guaranteed signalIds are continuous from 0 to x and x is know upfront
    ...
    private:
        std::bitset<MAX_NUMBER_OF_ACTIVE_CONDITION> mAlreadyConsumed;
    };
    struct SignalSample : SampleConsumed
    {
        ...
        InspectionValue mValue;
        InspectionTimestamp mTimestamp;
    };
    ...
    struct CanFrameSample : SampleConsumed
    {
        ...
        uint8_t mSize; /**< elements in buffer variable used. So if the raw can messages is only 3 bytes big this
                         uint8_t will be 3 and only the first three bytes of buffer will contain meaningful data. size
                         optimized sizeof(size)+sizeof(buffer) = 9 < sizeof(std:vector) = 24 so also for empty messages
                         its mor efficient to preallocate 8 bytes even if not all will be used*/
    ...
    public:
        FixedTimeWindowFunctionData( uint32_t size )
            : mWindowSizeMs( size )
            , mLastTimeCalculated( 0 )
            , mLastMin( std::numeric_limits<InspectionValue>::min() )
            , mLastMax( std::numeric_limits<InspectionValue>::max() )
            , mLastAvg( 0 )
            , mLastAvailable( false )
            , mPreviousLastMin( std::numeric_limits<InspectionValue>::min() )
            , mPreviousLastMax( std::numeric_limits<InspectionValue>::max() )
            , mPreviousLastAvg( 0 )
            , mPreviousLastAvailable( false )
            , mCollectingMin( std::numeric_limits<InspectionValue>::min() )
            , mCollectingMax( std::numeric_limits<InspectionValue>::max() )
            , mCollectingSum( 0 )
            , mCollectedSignals( 0 )
        {
        }
        InspectionTimestamp mWindowSizeMs;       /** <over which time is the window calculated */
        InspectionTimestamp mLastTimeCalculated; /** <the time the last window stopped*/

        // This value represent the newest window that was complete and calculated
        InspectionValue mLastMin;
        InspectionValue mLastMax;
        InspectionValue mLastAvg;
        bool mLastAvailable; /** <if this is false there is no data for this window. For example if the window is 30
                              * minutes long in the first 30 minutes of running FWE receiving the signal no data is
                              * available
                              */
        // This values represents the value before the newest window
        InspectionValue mPreviousLastMin;
        InspectionValue mPreviousLastMax;
        InspectionValue mPreviousLastAvg;
        bool mPreviousLastAvailable;
        ...
        InspectionValue mCollectingMin;
        InspectionValue mCollectingMax;
        InspectionValue mCollectingSum;
        uint32_t mCollectedSignals;
    ...
    {
        SignalHistoryBuffer()
            : mMinimumSampleIntervalMs( 0 )
            , mSize( 0 )
            , mCurrentPosition( mSize - 1 )
            , mCounter( 0 )
            , mLastSample( 0 )
            , mConditionsThatEvaluateOnThisSignal()
        {
        }
        ...
        uint32_t mMinimumSampleIntervalMs;
        ...
        uint32_t mSize; // minimum size needed by all conditions, buffer must be at least this big
        uint32_t mCurrentPosition; /**< position in ringbuffer needs to come after size as it depends on it */
        uint32_t mCounter;         /**< over all recorded samples*/
        InspectionTimestamp mLastSample;
    ...
    {
        CanFrameHistoryBuffer()
            : mFrameID( INVALID_CAN_FRAME_ID )
            , mChannelID( INVALID_CAN_CHANNEL_NUMERIC_ID )
            , mMinimumSampleIntervalMs( 0 )
            , mSize( 0 )
            , mCurrentPosition( mSize - 1 )
            , mCounter( 0 )
        ...
        CANRawFrameID mFrameID;
        CANChannelNumericID mChannelID;
        uint32_t mMinimumSampleIntervalMs;
        std::vector<struct CanFrameSample>
            mBuffer; // ringbuffer, Consider to move to raw pointer allocated with new[] if vector allocates too much
        uint32_t mSize;
        uint32_t mCurrentPosition; // position in ringbuffer
        uint32_t mCounter;
        InspectionTimestamp mLastSample;
    };
    ...
    struct ActiveCondition
    {
        ...
        InspectionTimestamp mLastTrigger;
     ...
     */
    EventID generateEventID( InspectionTimestamp timestamp );
    ...
    using SignalHistoryBufferCollection = std::vector<std::vector<SignalHistoryBuffer>>;
    ...
    std::shared_ptr<const TriggeredCollectionSchemeData> collectData( ActiveCondition &condition,
                                                                      uint32_t conditionId,
                                                                      InspectionTimestamp &newestSignalTimestamp );
    uint32_t mNextConditionToCollectedIndex;

    InspectionTimestamp mNextWindowFunctionTimesOut;
    Aws::IoTFleetWise::Platform::LoggingModule mLogger;
    DataReduction mDataReduction;
    bool mSendDataOnlyOncePerCondition;
};
