...
Aws::IoTFleetWise::DataManagement::IDecoderManifest::~IDecoderManifest()
{
}
Aws::IoTFleetWise::DataManagement::ICollectionScheme::~ICollectionScheme()
{
}
...
TEST_F( CollectionInspectionEngineTest, CollectRawCanFrames )
{
    ...
    uint64_t timestamp = 160000000;
    std::array<uint8_t, MAX_CAN_FRAME_BYTE_SIZE> buf = { 0xDE, 0xAD, 0xBE, 0xEF };
...
TEST_F( CollectionInspectionEngineTest, CollectWithAfterTime )
{
    ...
    timestamp += 1000;
    timestampT timestamp0 = timestamp;
    engine.addNewSignal( s3.signalID, timestamp, 2000.0 );
    engine.addNewSignal( s1.signalID, timestamp, -90.0 );
    engine.addNewSignal( s2.signalID, timestamp, -480.0 );
    ...
    timestamp += 1000;
    timestampT timestamp1 = timestamp;
    engine.addNewSignal( s3.signalID, timestamp, 3000.0 );
    ...
    timestamp += 500;
    timestampT timestamp2 = timestamp;
    ...
    timestamp += 500;
    timestampT timestamp3 = timestamp;
