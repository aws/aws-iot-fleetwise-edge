...
StreamManager::onChangeCollectionSchemeList(
    const std::shared_ptr<const Aws::IoTFleetWise::ActiveCollectionSchemes> &activeCollectionSchemes )
{
...
StreamManager::appendToStreams( const TriggeredCollectionSchemeData &data )
{
    ...
    if ( data.signals.empty() && data.canFrames.empty() )
    {
    ...
    for ( const auto &partition : mCampaigns[campaign].partitions )
    {
        ...
        // currently, only signals are partitions.
        // CAN frames will go in partition 0 (default).
        // DTC info is not currenlty supported.
        if ( partition.id == DEFAULT_PARTITION )
        {
            for ( auto frame : data.canFrames )
            {
                currChunk.canFrames.emplace_back( frame );
                currChunkSize++;
                if ( currChunkSize >= mTransmitThreshold )
                {
                    auto res = store( currChunk, partition );
...
StreamManager::store( const TriggeredCollectionSchemeData &data, const Partition &partition )
{
    ...
    auto append_or = partition.stream->append( aws::store::common::BorrowedSlice{ dataBuf.data(), dataBuf.size() },
                                               aws::store::stream::AppendOptions{ true, true } );
    if ( !append_or.ok() )
    {
...
{
    auto dataPtr = std::make_shared<TriggeredCollectionSchemeData>( data );
    mProtoWriter->setupVehicleData( dataPtr, data.eventID );

    // Add signals to the protobuf
    for ( const auto &signal : dataPtr->signals )
    {
        {
    ...
    // Add raw CAN frames to the protobuf
    for ( const auto &canFrame : dataPtr->canFrames )
    {
        mProtoWriter->append( canFrame );
    }

    // Add DTC info to the payload
    if ( dataPtr->mDTCInfo.hasItems() )
    {
        mProtoWriter->setupDTCInfo( dataPtr->mDTCInfo );
        const auto &dtcCodes = dataPtr->mDTCInfo.mDTCCodes;
