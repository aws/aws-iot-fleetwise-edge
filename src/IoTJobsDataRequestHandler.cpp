...
IoTJobsDataRequestHandler::IoTJobsDataRequestHandler(
    ...
    , mStreamForwarder( std::move( streamForwarder ) )
{
    ...
    receiveIotJobs->subscribeToDataReceived( [this]( const ReceivedConnectivityMessage &receivedMessage ) {
        onIotJobReceived( receivedMessage.buf, receivedMessage.size );
    } );
    receiveAcceptedJobDocuments->subscribeToDataReceived( [this]( const ReceivedConnectivityMessage &receivedMessage ) {
        onIotJobDocumentAccepted( receivedMessage.buf, receivedMessage.size );
    } );
    receiveRejectedJobDocuments->subscribeToDataReceived( []( const ReceivedConnectivityMessage &receivedMessage ) {
        onIotJobDocumentRejected( receivedMessage.buf, receivedMessage.size );
    } );
    receiveAcceptedPendingJobs->subscribeToDataReceived( [this]( const ReceivedConnectivityMessage &receivedMessage ) {
        onPendingJobsAccepted( receivedMessage.buf, receivedMessage.size );
    } );
    receiveRejectedPendingJobs->subscribeToDataReceived( [this]( const ReceivedConnectivityMessage &receivedMessage ) {
        onPendingJobsRejected( receivedMessage.buf, receivedMessage.size );
    } );
    receiveAcceptedUpdateJobs->subscribeToDataReceived( [this]( const ReceivedConnectivityMessage &receivedMessage ) {
        onUpdateJobStatusAccepted( receivedMessage.buf, receivedMessage.size );
    } );
    receiveRejectedUpdateJobs->subscribeToDataReceived( []( const ReceivedConnectivityMessage &receivedMessage ) {
        onUpdateJobStatusRejected( receivedMessage.buf, receivedMessage.size );
    } );
    receiveCanceledJobs->subscribeToDataReceived( [this]( const ReceivedConnectivityMessage &receivedMessage ) {
...
IoTJobsDataRequestHandler::onIotJobDocumentAccepted( const uint8_t *buf, size_t size )
{
    ...
    if ( reader.parse( std::string( buf, buf + size ), jobDocument ) )
    {
        ...
        if ( execution["status"] == "IN_PROGRESS" )
        {
            ...
            {
                FWE_LOG_ERROR( "CampaignArn value in the received job document does not match the arn of a "
