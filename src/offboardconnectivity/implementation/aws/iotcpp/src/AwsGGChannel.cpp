...
AwsGGChannel::AwsGGChannel( IConnectivityModule *connectivityModule,
                            ...
                            std::shared_ptr<GreengrassCoreIpcClient> &ggConnection )
...
ConnectivityError
AwsGGChannel::sendBuffer( const std::uint8_t *buf, size_t size, struct CollectionSchemeParams collectionSchemeParams )
{
    std::lock_guard<std::mutex> connectivityLock( mConnectivityMutex );
    if ( !isTopicValid() )
    {
...
ConnectivityError
AwsGGChannel::sendFile( const std::string &filePath, size_t size, struct CollectionSchemeParams collectionSchemeParams )
{
    std::lock_guard<std::mutex> connectivityLock( mConnectivityMutex );
    if ( !isTopicValid() )
    {
...
AwsGGChannel::unsubscribe()
{
    ...
    {
        mSubscribeOperation.get()->Close().wait();
        mSubscribed = false;
        return true;
    }
    return false;
