...
AwsIotChannel::subscribe()
{
    ...
    auto onSubAck = [&]( Mqtt::MqttConnection &mqttConnection,
        ...
        {
            if ( packetId == 0u || QoS == AWS_MQTT_QOS_FAILURE )
            {
    ...
    connection->Subscribe( mTopicName.c_str(), AWS_MQTT_QOS_AT_LEAST_ONCE, onMessage, onSubAck );
...
AwsIotChannel::send( const std::uint8_t *buf, size_t size, struct CollectionSchemeParams collectionSchemeParams )
{
    ...
    if ( !isAliveNotThreadSafe() )
    {
        mLogger.warn( "AwsIotChannel::send", "No alive MQTT Connection." );
    ...
    connection->Publish( mTopicName.c_str(), AWS_MQTT_QOS_AT_MOST_ONCE, false, payload, onPublishComplete );
    return ConnectivityError::Success;
}
