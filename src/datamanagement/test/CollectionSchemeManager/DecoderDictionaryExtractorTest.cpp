...
TEST( CollectionSchemeManagerTest, DecoderDictionaryExtractorTest )
{
    ...
    std::unordered_map<SignalID, PIDSignalDecoderFormat> signalIDToPIDDecoderFormat = {
        { 0x1000, PIDSignalDecoderFormat( 4, CURRENT_STATS, 0x14, 0.0125, -40.0, 0, 2, 0, 8 ) },
        { 0x1001, PIDSignalDecoderFormat( 4, CURRENT_STATS, 0x14, 0.0125, -40.0, 2, 2, 0, 8 ) },
        { 0x7005, PIDSignalDecoderFormat( 10, CURRENT_STATS, 0x70, 1.0, 0.0, 9, 1, 0, 2 ) },
        { 0x7006, PIDSignalDecoderFormat( 10, CURRENT_STATS, 0x70, 1.0, 0.0, 9, 1, 2, 2 ) } };
    ...
    ASSERT_TRUE( decoderDictionaryMap.find( RAW_SOCKET ) != decoderDictionaryMap.end() );
    ASSERT_TRUE( decoderDictionaryMap.find( OBD ) != decoderDictionaryMap.end() );
    auto &decoderDictionary = decoderDictionaryMap[RAW_SOCKET];
    ...
    ASSERT_EQ( decoderDictionaryMap[OBD]->signalIDsToCollect.size(), 4 );
    const auto &obdPidDecoderDictionary = decoderDictionaryMap[OBD]->canMessageDecoderMethod;
    ...
    std::map<NetworkChannelProtocol, std::shared_ptr<CANDecoderDictionary>> decoderDictionaryMapNew;
    test.decoderDictionaryExtractor( decoderDictionaryMapNew );
    ASSERT_TRUE( decoderDictionaryMapNew.find( RAW_SOCKET ) != decoderDictionaryMapNew.end() );
    // OBD is only included in CollectionScheme 2 and it's already expired
    ASSERT_TRUE( decoderDictionaryMapNew.find( OBD ) == decoderDictionaryMapNew.end() );
    decoderDictionary = decoderDictionaryMapNew[RAW_SOCKET];
    ...
    // Invoke Decoder Dictionary Extractor function
    std::map<NetworkChannelProtocol, std::shared_ptr<CANDecoderDictionary>> decoderDictionaryMap2;
    test2.decoderDictionaryExtractor( decoderDictionaryMap2 );
    ASSERT_TRUE( decoderDictionaryMap2.find( RAW_SOCKET ) != decoderDictionaryMap2.end() );
    ASSERT_TRUE( decoderDictionaryMap2.find( OBD ) != decoderDictionaryMap2.end() );
}

/**  @brief
 * This test aims to test PM's functionality to invoke all Network Channel Consumer on Decoder Dictionary Update
 * step1:
 * Two Network Channel Consumer registered as listener
...
TEST( CollectionSchemeManagerTest, DecoderDictionaryUpdaterTest )
{
    ...
    // Mock the Network Channel Binder
    std::shared_ptr<NetworkChannelBinderMock> binderMockPtr;
    binderMockPtr.reset( new NetworkChannelBinderMock() );
    ...
    decoderDictionaryMap.emplace( OBD, std::make_shared<CANDecoderDictionary>() );
    testPtr->decoderDictionaryUpdater( decoderDictionaryMap );
...
TEST( CollectionSchemeManagerTest, DecoderDictionaryExtractorFirstRawFrameThenSignal )
{
    ...
    ASSERT_TRUE( decoderDictionaryMap.find( RAW_SOCKET )
                     ->second->canMessageDecoderMethod.find( canIDTranslator.getChannelNumericID( "10" ) ) !=
                 decoderDictionaryMap.find( RAW_SOCKET )->second->canMessageDecoderMethod.cend() );
    auto decoderMethod = decoderDictionaryMap.find( RAW_SOCKET )
                             ->second->canMessageDecoderMethod.find( canIDTranslator.getChannelNumericID( "10" ) );
