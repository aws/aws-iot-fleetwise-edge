...
/**
 * @brief Implementation of base class destructor required.
 */
IDecoderManifest::~IDecoderManifest()
{
}
...
DecoderManifestIngestion::getNetworkProtocol( SignalID signalID ) const
{
    ...
    if ( mSignalToNetworkChannelProtocol.count( signalID ) > 0 )
    {
        // It exists, so return it
        return mSignalToNetworkChannelProtocol.at( signalID );
    }
    return INVALID_PROTOCOL;
}
...
DecoderManifestIngestion::build()
{
    ...
    for ( int i = 0; i < mProtoDecoderManifest.can_signals_size(); i++ )
    {
        ...
        mSignalToNetworkChannelProtocol[canSignal.signal_id()] = RAW_SOCKET;
    ...
    for ( int i = 0; i < mProtoDecoderManifest.obd_pid_signals_size(); i++ )
    {
        ...
        mSignalToNetworkChannelProtocol[pidSignal.signal_id()] = OBD;
