...
DataSenderManagerWorkerThread::DataSenderManagerWorkerThread( CANInterfaceIDTranslator &canIDTranslator,
    ...
    : mDataSenderManager( std::move( mqttSender ), std::move( payloadManager ), canIDTranslator, maxMessageCount )
    , mConnectivityModule( std::move( connectivityModule ) )
{
    mPersistencyUploadRetryIntervalMs = persistencyUploadRetryInterval;
}

bool
DataSenderManagerWorkerThread::init( const std::shared_ptr<CollectedDataReadyToPublish> &collectedDataQueue )
{
...
DataSenderManagerWorkerThread::doWork( void *data )
{
    ...
    while ( !sender->shouldStop() )
    {
        ...
        {
            sender->mWait.wait( Platform::Linux::Signal::WaitWithPredicate );
        ...
        auto consumedElements = sender->mCollectedDataQueue->consume_all(
