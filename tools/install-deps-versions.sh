#!/bin/bash
# Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.

export VERSION_BOOST="1.84.0"
# This should be a commit from https://github.com/moritz-wundke/Boost-for-Android that provides
# support for the desired boost version.
export VERSION_BOOST_FOR_ANDROID="83ba7174dea6e0595085dc30051eb17747e7260a"
export VERSION_JSON_CPP="1.9.5"
export VERSION_PROTOBUF="3.21.12"
export VERSION_PROTOBUF_RELEASE="v21.12"
export VERSION_CURL="7.86.0"
export VERSION_CURL_RELEASE="curl-7_86_0"
export VERSION_AWS_SDK_CPP="1.11.284"
# MAKE SURE THE CRT VERSION IN AWS_SDK_CPP IS THE SAME AS IN AWS_IOT_SDK_CPP
export VERSION_AWS_IOT_DEVICE_SDK_CPP_V2="v1.32.2"
export VERSION_FAKETIME="v0.9.10"
export VERSION_TINYXML2="6.0.0"
export VERSION_FAST_CDR="v1.0.24"
export VERSION_GOOGLE_TEST="v1.15.2"
export VERSION_GOOGLE_BENCHMARK="v1.6.1"
export VERSION_ANDROID_CMDLINE_TOOLS="9123335"
export VERSION_ANDROID_NDK="23.1.7779620"
export VERSION_ANDROID_API="21"
export VERSION_ANDROID_SDK="33"
export VERSION_ANDROID_BUILD_TOOLS="30.0.3"
export VERSION_CMAKE="3.22.1" # Provided by Ubuntu 22.04, version here used for Android build
export VERSION_SNAPPY="1.1.8" # Provided by Ubuntu 22.04, version here used for Android build
export VERSION_OPENSSL="3.0.2" # Provided by Ubuntu 22.04, version here used for Android build
export VERSION_ION_C="v1.1.2"
export VERSION_VSOMEIP="3.5.1"
export VERSION_COMMON_API="3.2.4"
export VERSION_COMMON_API_SOMEIP="3.2.4"
export VERSION_COMMON_API_GENERATOR="3.2.15"
export VERSION_PYBIND11="v2.10.0"
export VERSION_DEVICE_STORELIBRARY_CPP="1.0.0"
export VERSION_MICROPYTHON="v1.23.0"
