...
if ! command -v cansend > /dev/null; then
    ...
    apt install -y -o DPkg::Lock::Timeout=120 can-utils
fi

# For EC2, the SocketCAN modules vcan and can-gw are included in a separate package:
if uname -r | grep -q aws; then
    apt update -o DPkg::Lock::Timeout=120
    apt install -y -o DPkg::Lock::Timeout=120 linux-modules-extra-aws
fi
...
if modinfo $MODULE &> /dev/null ; then
    ...
    apt update -o DPkg::Lock::Timeout=120
    apt install -y -o DPkg::Lock::Timeout=120 build-essential dkms git
