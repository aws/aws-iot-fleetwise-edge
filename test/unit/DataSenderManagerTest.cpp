...
{
public:
    DataSenderManagerTest()
        : mPersistency( createCacheAndPersist() )
        , mPayloadManager( std::make_unique<PayloadManager>( mPersistency ) )
#ifdef FWE_FEATURE_VISION_SYSTEM_DATA
    ...
    SetUp() override
    {
        ...
        mDataSenders[SenderDataType::TELEMETRY] = std::move( telemetryDataSender );
#ifdef FWE_FEATURE_VISION_SYSTEM_DATA
        ...
        auto ionWriter = std::make_unique<StrictMock<Testing::DataSenderIonWriterMock>>();
        mIonWriter = ionWriter.get();
        auto visionSystemDataSender =
            std::make_unique<VisionSystemDataSender>( mUploadedS3Objects, mS3Sender, std::move( ionWriter ), "" );
        mVisionSystemDataSender = visionSystemDataSender.get();
        mDataSenders.emplace( SenderDataType::VISION_SYSTEM, std::move( visionSystemDataSender ) );
#endif
#ifdef FWE_FEATURE_REMOTE_COMMANDS
        ...
        ON_CALL( mCommandResponseSender, mockedSendBuffer( _, _, _, _ ) )
            .WillByDefault( InvokeArgument<3>( ConnectivityError::Success ) );
        auto commandResponseDataSender = std::make_unique<CommandResponseDataSender>( mCommandResponseSender );
        mCommandResponseDataSender = commandResponseDataSender.get();
        mDataSenders.emplace( SenderDataType::COMMAND_RESPONSE, std::move( commandResponseDataSender ) );
#endif
#ifdef FWE_FEATURE_LAST_KNOWN_STATE
        ON_CALL( mLastKnownStateMqttSender, mockedSendBuffer( mLastKnownStateDataTopic, _, _, _ ) )
            .WillByDefault( InvokeArgument<3>( ConnectivityError::Success ) );
        auto lastKnownStateDataSender =
            std::make_unique<LastKnownStateDataSender>( mLastKnownStateMqttSender, mMaxMessagesPerPayload );
        mLastKnownStateDataSender = lastKnownStateDataSender.get();
        mDataSenders.emplace( SenderDataType::LAST_KNOWN_STATE, std::move( lastKnownStateDataSender ) );
#endif

        mDataSenderManager = std::make_unique<DataSenderManager>( mDataSenders, mPayloadManager.get() );
    }
    ...
    void
    processCollectedData( const DataToSend &data )
    {
        mDataSenderManager->processData( data );
    }

#ifdef FWE_FEATURE_REMOTE_COMMANDS
    static std::string
    getCommandResponseTopic( const std::string &commandID )
    {
...
protected:
    ...
    TriggeredCollectionSchemeData mTriggeredCollectionSchemeData;
    ...
    std::unique_ptr<PayloadManager> mPayloadManager;
...
#ifdef FWE_FEATURE_VISION_SYSTEM_DATA
    TriggeredVisionSystemData mTriggeredVisionSystemData;
    VisionSystemDataSender *mVisionSystemDataSender;
    StrictMock<Testing::S3SenderMock> mS3Sender;
    StrictMock<Testing::DataSenderIonWriterMock> *mIonWriter;
    std::shared_ptr<ActiveCollectionSchemes> mActiveCollectionSchemes;
    DataSenderQueue mUploadedS3Objects;
#endif
#ifdef FWE_FEATURE_REMOTE_COMMANDS
    StrictMock<Testing::SenderMock> mCommandResponseSender;
    CommandResponseDataSender *mCommandResponseDataSender;
#endif
#ifdef FWE_FEATURE_LAST_KNOWN_STATE
    std::string mLastKnownStateDataTopic = "$aws/iotfleetwise/vehicles/thing-name/last_known_states/data";
    StrictMock<Testing::SenderMock> mLastKnownStateMqttSender;
    LastKnownStateDataSender *mLastKnownStateDataSender;
#endif
#ifdef FWE_FEATURE_STORE_AND_FORWARD
    StrictMock<Testing::StreamManagerMock> mStreamManagerMock;
#endif
};
...
TEST_F( DataSenderManagerTest, ProcessSingleRawDataSignal )
{
    ...
    EXPECT_CALL( *mIonWriter, getStreambufBuilder() ).WillOnce( []() {
        return std::make_unique<Testing::StringbufBuilder>( "fake ion file" );
    } );
    ...
    EXPECT_CALL( mS3Sender, sendStream( _, s3UploadMetadata, std::string( "s3/prefix/raw-data/579-1000000.10n" ), _ ) )
        .WillOnce( WithArgs<0, 3>( [&sentStream]( std::unique_ptr<StreambufBuilder> streambufBuilder,
            ...
            sentStream = streambufBuilder->build();
...
TEST_F( DataSenderManagerTest, ProcessRawDataSignalFailureWithPersistency )
{
    ...
    EXPECT_CALL( *mIonWriter, getStreambufBuilder() ).WillOnce( [payload]() {
        return std::make_unique<Testing::StringbufBuilder>( payload );
    } );
    EXPECT_CALL( mS3Sender, sendStream( _, s3UploadMetadata, std::string( "s3/prefix/raw-data/579-1000000.10n" ), _ ) )
        // Can't use DoAll(InvokeArgument, Return) here: https://stackoverflow.com/a/70886530
...
TEST_F( DataSenderManagerTest, PersistencyForTelemetryMultipleFiles )
{
    ...
    for ( ; ( estimatedSize + 20 ) < maxSize; numberOfSignals++ )
    {
...
TEST_P( DataSenderManagerTestWithAllCommandStatuses, ProcessSingleCommandResponse )
{
    ...
    mDataSenderManager->processData( CommandResponse( "command123", status.internalStatus, 0x1234, "status456" ) );
...
TEST_F( DataSenderManagerTest, PersistencyForCommandResponsePersistAgainOnFailure )
{
    ...
    mDataSenderManager->processData(
        CommandResponse( "command123", CommandStatus::EXECUTION_FAILED, 0x1234, "status456" ) );
...
{
    std::unordered_map<SenderDataType, std::unique_ptr<DataSender>> dataSenders;
    mDataSenderManager = std::make_unique<DataSenderManager>( std::move( dataSenders ), mPayloadManager.get() );

    EXPECT_CALL( mCommandResponseSender, mockedSendBuffer( getCommandResponseTopic( "command123" ), _, _, _ ) )
        .Times( 0 );

    mDataSenderManager->processData( CommandResponse( "command123", CommandStatus::SUCCEEDED, 0x1234, "status456" ) );
...
TEST_P( DataSenderManagerTestWithAllSignalTypes, ProcessSingleLastKnownStateSignal )
{
    ...
    LastKnownStateCollectedData collectedData;
...
TEST_F( DataSenderManagerTest, ProcessMultipleLastKnownStateSignalsBeyondTransmitThreshold )
{
    ...
    collectedData = LastKnownStateCollectedData();
    collectedData.triggerTime = 1000;
...
TEST_F( DataSenderManagerTest, InvalidSenderWhenProcessingLastKnownState )
{
    ...
    mDataSenderManager->processData( LastKnownStateCollectedData() );
...
#ifdef FWE_FEATURE_STORE_AND_FORWARD
TEST_F( DataSenderManagerTest, StoreAndForward )
{
    auto signal1 = CollectedSignal( 1234, 789654, 40.5, SignalType::DOUBLE );
    mTriggeredCollectionSchemeData.signals.push_back( signal1 );
    mTriggeredCollectionSchemeData.metadata.compress = true;
    ...
    EXPECT_CALL( mStreamManagerMock, appendToStreams( _ ) )
        .WillOnce( Return( Store::StreamManager::ReturnCode::EMPTY_DATA ) );
    processCollectedData( mTriggeredCollectionSchemeData );

    EXPECT_CALL( mStreamManagerMock, appendToStreams( _ ) )
        .WillOnce( Return( Store::StreamManager::ReturnCode::END_OF_STREAM ) );
    processCollectedData( mTriggeredCollectionSchemeData );
}
#endif
