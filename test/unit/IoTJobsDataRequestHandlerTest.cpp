...
protected:
    ...
    SetUp() override
    {
        ...
        mTopicConfig = std::make_unique<TopicConfig>( "clientIdTest", topicConfigArgs );
        ...
        mStreamManager = std::make_shared<StrictMock<Testing::StreamManagerMock>>( protoWriter );

        mMqttSender = std::make_shared<StrictMock<Testing::SenderMock>>( *mTopicConfig );
        ...
        mStreamForwarder =
            std::make_shared<StrictMock<Testing::StreamForwarderMock>>( mStreamManager, mTelemetryDataSender );
    }
    ...
    std::shared_ptr<StrictMock<Testing::StreamManagerMock>> mStreamManager;
    std::shared_ptr<TelemetryDataSender> mTelemetryDataSender;
    std::shared_ptr<StrictMock<Testing::StreamForwarderMock>> mStreamForwarder;
...
TEST_F( IoTJobsDataRequestHandlerTest, IoTJobsDataRequestHandler )
{
    EXPECT_CALL( *mStreamForwarder, registerJobCompletionCallback( _ ) )
        .Times( 1 )
        .WillRepeatedly( Invoke( [this]( Store::StreamForwarder::JobCompletionCallback jobCompletionCallback ) -> void {
    ...
    WAIT_ASSERT_TRUE( static_cast<IReceiver *>( mReceiverIotJob.get() )->isAlive() );
    WAIT_ASSERT_TRUE( static_cast<IReceiver *>( mReceiverJobDocumentAccepted.get() )->isAlive() );
    WAIT_ASSERT_TRUE( static_cast<IReceiver *>( mReceiverJobDocumentRejected.get() )->isAlive() );
    WAIT_ASSERT_TRUE( static_cast<IReceiver *>( mReceiverPendingJobsAccepted.get() )->isAlive() );
    WAIT_ASSERT_TRUE( static_cast<IReceiver *>( mReceiverPendingJobsRejected.get() )->isAlive() );
    WAIT_ASSERT_TRUE( static_cast<IReceiver *>( mReceiverUpdateIotJobStatusRejected.get() )->isAlive() );
    WAIT_ASSERT_TRUE( static_cast<IReceiver *>( mReceiverCanceledIoTJobs.get() )->isAlive() );
