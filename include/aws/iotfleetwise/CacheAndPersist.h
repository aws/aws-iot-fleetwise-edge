...
private:
     ...
     *                     FILESYSTEM_ERROR in case of any file I/O errors.
     */
    ErrorCode write( const uint8_t *bufPtr, size_t size, const std::string &path );
     ...
     *                     FILESYSTEM_ERROR in case of any file I/O errors.
     */
    ErrorCode read( uint8_t *const readBufPtr, size_t size, const std::string &path ) const;

    /**
     * @brief Try to read the checksum for the given file.
     * @param path The path for the file with the content. Note that this is not the file containing the checksum.
     * @param result The checksum read from the file if any.
     * @return ErrorCode   SUCCESS if the read is successful. In such case, the caller should use the result to verify
     *                     the content of the file.
     *                     EMPTY if the checksum doesn't exist.
     *                     FILESYSTEM_ERROR in case of any file I/O errors.
     */
    static ErrorCode readChecksumForFile( const std::string &path, std::string &result );

    /**
     * @brief Write the checksum for the given file.
     * @param path The path for the file with the content. Note that this is not the file containing the checksum.
     * @param checksum The checksum calculated from the content of the given file.
     * @return ErrorCode   SUCCESS if the write is successful.
     *                     FILESYSTEM_ERROR in case of any file I/O errors.
     */
    static ErrorCode writeChecksumForFile( const std::string &path, const std::string &checksum );

    /**
     * @brief Give the filename where a checksum should be saved.
     * @param path The path for the original file with the content. This will be used to derive the checksum filename.
     * @return The checksum filename.
     */
    static std::string getChecksumFilename( const std::string &path );
     ...
     *                     FILESYSTEM_ERROR in case of any file I/O errors.
     */
    static ErrorCode erase( const std::string &path );
